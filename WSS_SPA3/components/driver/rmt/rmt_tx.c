...
//static void rmt_tx_start(rmt_channel_handle_t channel);
...
esp_err_t rmt_transmit(rmt_channel_handle_t channel, rmt_encoder_t *encoder, const void *payload, size_t payload_bytes, const rmt_transmit_config_t *config)
{
    ...
    if (atomic_compare_exchange_strong(&channel->fsm, &expected_fsm, RMT_FSM_RUN_WAIT)) {
        ...
        if (xQueueReceive(tx_chan->trans_queues[RMT_TX_QUEUE_PROGRESS], &t, 0) == pdTRUE) {
            ...
            rmt_tx_do_transaction(tx_chan, t); // EDIT - can also disable transmission, but at a higher level than rmt_ll_tx_enable_loop() or rmt_ll_tx_start().
        } else {
...
{   
    ...
    //rmt_ll_tx_enable_loop(hal->regs, channel_id, t->loop_count != 0); // EDIT -  Controls start of transmission on ESP32. Disable here and call later in main()/custom function instead.
    ...
    //rmt_ll_tx_start(hal->regs, channel_id); // EDIT - Controls start only on ESP32-S3. ESP32 is seemingly controlled above by rmt_ll_tx_enable_loop().
    portEXIT_CRITICAL_SAFE(&channel->spinlock);
...
{   
    ...
    if (trans_desc) {
        ...
        if (trans_desc->remain_loop_count) {
            ...
            rmt_ll_tx_start(hal->regs, channel_id); // EDIT - unused for ESP32 and ESP32-S3
            portEXIT_CRITICAL_ISR(&channel->spinlock);
            return need_yield;
        }
...
// **********************************************************
// ------------------------------ CUSTOM CHANNEL START FUNCTION
// EDIT
//static
void rmt_tx_start(rmt_channel_handle_t channel1, rmt_channel_handle_t channel2)
{   // Input a pair of RMT channels to be started in quick succession
    ...
    // original call inside rmt_tx_do_transaction():
